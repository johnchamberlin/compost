...
def annotate_peaks(peaks,
    ...
    peak_annotation,
    gtfpath,
    homopolymerpath,libtype="10x",min_reads=50,trans_spliced=False):
    """
    ...
    if trans_spliced:
        ...
        (
            (
                (peakann.peak_type == "tss")
                & (peakann.median_clip > 23)
                & (peakann.Score > min_reads)
                & (peakann.frac_GGGG > 0.6)
            )
            | (
                (peakann.peak_type == "tss")
                & (peakann.median_clip > 45)
                & (peakann.Score > min_reads)
            )
            | (
                (peakann.peak_type == "pas")
                & (peakann.median_clip > 15)
                & (peakann.median_clip < 30)
                & (peakann.Score > min_reads)
                & (peakann.NumberOverlaps == 0)
            )
        ),
