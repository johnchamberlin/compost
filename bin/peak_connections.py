...
# todo: annotate PAS peaks for longer A homopolymer
# here is the AI function from copilot lmao:
# def annotate_pas_peaks(pas_peaks):
#     for peak in pas_peaks.itertuples():
#         # Get the sequence of the peak
#         seq = peak.Sequence
#         # Find all runs of A's in the sequence
#         a_runs = [len(run) for run in seq.split("A") if run]
#         # If there are any runs of A's, annotate the peak
#         if a_runs:
#             peak.A_homopolymer_length = max(a_runs)
#         else:
#             peak.A_homopolymer_length = 0
#     return pas_peaks
